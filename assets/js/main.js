	...
	var methods = {
		init: function(options){
			var options = {
				...
				current : $('.curr'),
				all : $('.quant'),
				viewport : $('.ts_viewport'),
				text : $('.ts_text'),
				pBtn : $('.previous'),
				nBtn : $('.to-next'),
				// Getting amount of pages
				getAll : function(){
					getter = parseInt(options.text.height() / options.viewport.height());
					if(options.text.height() % options.viewport.height() > 0){
						getter = getter + 1;
					}
					return getter;
				},
				slideNext: function(){
					var self = $(this);
					options.pBtn.removeClass('disabled');
					if(!(options.counter == options.all.html())){
						self.removeClass('disabled');
						options.offsetText += options.viewport.height();
						options.counter += 1;
						options.text.css({
							'top': -options.offsetText +'px'
						});
						options.current.html(options.counter);
					}
					if(options.counter == options.all.html()){
						self.addClass('disabled');
					}
					return options.offsetText;
				},

				slidePrev: function(){
					var self = $(this);
					options.nBtn.removeClass('disabled');
				...
				clearOnResize: function(){
					options.text.removeAttr('style');
					options.counter = 1;
					options.current.html('1');
					offsetText = 0;
					options.all.html(options.getAll());
				}
			}
			...
			options.pBtn.on('click', function(event){
				event.preventDefault();
				options.slidePrev();
			});
			options.nBtn.on('click', function(event){
				event.preventDefault();
				options.slideNext();
			});
			options.current.html('1');
...
(function($){
	var $window     = $(window),
		...
		$document   = $(document),
		$body       = $('body'),
		$tabLinks   = $('.tabs-head-list > ul > li'),
		$tabLink    = $('.js_tab-link > a.parent-cat'),
		$tabContent = $('.catalog-tabs-content'),
		$dHeader    = $('.desktop-header'),
		$dFooter    = $('.footer-desktop'),
		$tltpLink   = $('.js_tooltip-link'),
		$catAccItem = $('.mobile-acc-cat'),
		$menuBtn    = $('.js_mobile-nav'),
		$menuWrap   = $('.mobile-nav-wrap'),
		$mobNavAc   = $('.js_nav-ac'),
		$closeMenu  = $('.js_close-menu'),
		$productDtl = $('.js_fullscreen'),
		$navSlider  = $('.js_nav-slider'),
		$pSlider    = $('.js_pSlider'),
		$pDescr     = $('.js_scrollable'),
		$gridTablnk = $('.add-tab-head li a'),
		$gridTabcnt = $('.add-tabs-content .prod-grid'),
		$faqLink    = $('.js_faqHead'),
		$reviewLink = $('.reviews-acc-head li a'),
		$revParent  = $('.reviews-acc-head li'),
		$revCont   	= $('.acc-content'),
		$mapBlock   = $('#map'),
		$fullscreen = $('.fullscreen'),
		$comboCheck =  $('.js_require'),
		$searchBtn  = $('.js_search-btn'),
		$mSearchBtn  = $('.js_mSearch-btn'),
		cardHeight  = function(){
			neededHeight = $winHeight - $productDtl.offset().top+ 'px';
			$productDtl.css('height', neededHeight);
		},
		fullScreenCard = function(){
			if($productDtl.length && $winWidth > 1280 && $winHeight > 650){cardHeight();}
		},
		pCardAcc = function(){
			$pTabLink = $('.ptab-head li a');
			$pTabCont = $('.ptab-content');
			var anch = $pTabLink.parent().data('type');

			$pTabLink.on('click', function(e){
				e.preventDefault();
				self = $(this);
				anch = self.parent().data('type');
				if(!self.closest('li').hasClass('active')){
					$pTabLink.parent().removeClass('active');
					$pTabCont.removeClass('active');
					self.parent().addClass('active');
					$pTabCont.each(function(i){
						if($(this).data('type') == anch){
	...
	fullScreenCard();
	pCardAcc();
	if($productDtl.length && $winWidth > 1280 && $winHeight > 768){cardHeight();}
	/* =========== Click handlers ================== */
	$document.on('click','.js_mobile-phones', function(e){
		e.preventDefault();
		$(this).next('ul').toggleClass('showed');
	});
	$document.on('click', '.js_mobile-personal', function(e){
		e.preventDefault();
		$(this).next('ul').toggleClass('showed');
	});
	$document.on('click', function(e){
		if(!$(e.target).closest('.m-search').length && !$searchBtn.parent().hasClass('no-input')) {
			$searchBtn.parent().toggleClass('no-input');
		}
		if(!$(e.target).closest('.mob-search').length && !$mSearchBtn.parent().hasClass('no-input')) {
			$mSearchBtn.parent().toggleClass('no-input');
		}
	});
	$gridTablnk.on('click', function(e){
		e.preventDefault();
		$this = $(this);
		var anch = $this.data('type')

		if(!$this.closest('li').hasClass('active')){
			$('.add-tab-head li').removeClass('active');
			$this.closest('li').addClass('active')
			$gridTabcnt.each(function(i){
				if ($(this).data('type') === anch) {
					$gridTabcnt.removeClass('active');
					$(this).addClass('active');
				}
			});
		}
	...
	$searchBtn.on('click', function(e){
		var $self = $(this);
		if($self.parent().hasClass('no-input')){
			e.preventDefault();
			$self.parent().toggleClass('no-input');
		}
	});
	...
	$tabLink.each(function(i){
		...
		$this.on('click', function(e){
			e.preventDefault();
			if($winWidth > 768) {
				var attrType = $(this).closest('li').data('type');
				$tabLinks.removeClass('tab-active');
				$tabContent.removeClass('tab-active');
				$(this).closest('.js_tab-link').addClass('tab-active');
				...
				$this = $(this);
				if(!$this.closest('li').find('.mobile-acc-cat').hasClass('active')){
					$catAccItem.slideUp();
					$catAccItem.removeClass('active');
					$this.closest('li').find('.mobile-acc-cat').slideDown().addClass('active');
				} else {
					$this.closest('li').find('.mobile-acc-cat').slideUp().removeClass('active');
				}	
			}
		});
	});
	$faqLink.on('click', function(e){
		e.preventDefault();
		var $this = $(this),
			attrAnch = $this.data('type');

		if(!$this.hasClass('active')){
			$faqLink.removeClass('active');
			$this.addClass('active');
			$('.answer').removeClass('active');
			$body.find('.answer').each(function(i){
				if($(this).data('type') === attrAnch) {
					$(this).addClass('active');
				}
			})
		}
	...
	$reviewLink.on('click', function(e){
		...
		if(!$this.parent('li').hasClass('active')){
			...
			$revParent.removeClass('active');
			$this.parent('li').addClass('active');
			$this.addClass('active');
			$revCont.removeClass('active');
			$revCont.each(function(i){
				if($(this).data('type') === attrAnch){
					$(this).addClass('active');
				};
			});
	...
	$mobNavAc.on('click', function(e){
		e.preventDefault();
		$this = $(this);
		if(!$this.closest('li').find('.second-level').hasClass('active')){
			$('.second-level').slideUp().removeClass('active');
			$this.closest('li').find('.second-level').slideDown().addClass('active');
		} else {
			$this.closest('li').find('.second-level').slideUp().removeClass('active');
		}
	});

	$('.js_slapped-filter').on('click', function(e){
		e.preventDefault();
		$(this).closest('.filter').toggleClass('slapped');
		if($(this).closest('.filter').hasClass('slapped')){
	...
	$comboCheck.on('change', function(){
		var self = $(this);
		if(self.prop("checked")){
			self.closest('.with-input').find('input[type="text"]').removeAttr('disabled');
		} else {
			self.closest('.with-input').find('input[type="text"]').attr('disabled', 'disabled');
	...
	function responseTextHeight(){
	    var wrapHeight = $('.product-info').outerHeight();
	    var countedHeight = wrapHeight - $('.product-name').height() - $('.information').height() - 100 + 'px';
	    if(!$('.information').hasClass('active')){
	    	countedHeight = parseInt(countedHeight) + $('.information').height() + 'px';
	    	$('.product-text-block .text-wrap').css('height', countedHeight);
	    }
	    else {
	    	$('.product-text-block .text-wrap').css('height', countedHeight);
	    }
	...
	if($fullscreen.length && $winWidth > 1024){
		$fullscreen.css('min-height', function(){
			var contactHeight = $winHeight - $dHeader.height() - $dFooter.height();
			return contactHeight;
		})
	}	
	$('.js_select').styler();
	$tltpLink.on('mouseover', function(e){
		e.preventDefault();
		$body.find('.tooltip-block').show();
	});
	$tltpLink.on('mouseleave', function(e){
		e.preventDefault();
		$body.find('.tooltip-block').hide();
	});
	$('.search-1 input').on('focus', function(){
		$(this).closest('.search-1').addClass('focused');
	});
	$('.search-1 input').on('focusout', function(){
		$(this).closest('.search-1').removeClass('focused');
	});
	$('.js_mh').matchHeight({
		byRow: false
	});
	function isMobile(){
		var x = true ? $winWidth < 1024 : false;
		return x;
	}
	$pDescr.mCustomScrollbar({
		setHeight: '100%'
	});
	$navSlider.slick({
		arrows: true,
		prevArrow: '<a href="#" class="slick-prev"><i class="icon-arrowleft"></i></a>',
		nextArrow: '<a href="#" class="slick-next"><i class="icon-arrowleft"></i></a>',
		infinite: false,
		asNavFor: '.js_pSlider',
		slidesToShow: 4,
		slidesToSCroll: 1,
	...
	$pSlider.slick({
		arrows: false,
		infinite: false,
		asNavFor: '.js_nav-slider',
		slidesToShow: 1,
		slidesToSCroll: 1,
		lazyLoad: 'ondemand'
	});
	if($('.js_pSlider').length){
		$pSlider.slickLightbox({
		   src: 'src',
		   itemSelector: '.img-hold img'
		});
	}
	...
	$window.on('scroll', function(){
		$winTop = $window.scrollTop();
		if(!isMobile()){
				if($winTop > $dHeader.height()){
				$dHeader.addClass('slapped');
			} else if($winTop < $dHeader.height()) {
				$dHeader.removeClass('slapped');
			}
		}
	});
	/* =========== Modals              ============== */
	$('.js_consult').on('click', function(e){
		e.preventDefault();
	...
	$('.js_doc').on('click', function(e){
		e.preventDefault();
		$('.doc-modal').arcticmodal({
			overlay: {
		        css: {
		            background: 'rgba(255,255,255, .8)' 
		        }
	...
	$('.js_cost').on('click', function(e){
		e.preventDefault();
		$('.cost-modal').arcticmodal({
			overlay: {
		        css: {
		            background: 'rgba(255,255,255, .8)' 
		        }
	...
	$window.on('resize', function(){
		$winWidth = $(window).width();
		$winHeight  = $window.innerHeight();
		if($winWidth < 1024 && $fullscreen.length){
			$fullscreen.css('min-height', '100%');
		}
		fullScreenCard();
		isMobile();
		$searchBtn.parent().addClass('no-input');
		responseTextHeight();
	});
...
function initMap() {
	...
	var coordLat = + mapBlock.getAttribute('data-latitude');
	var coordLng = + mapBlock.getAttribute('data-longitude');
	var myLatLng = {lat: coordLat, lng: coordLng};
	...
	var image = '/assets/img/icon-location.png';
...
function dealersMaps() {
	var maps    = $('.map'),
        markers = [],
	    mapType,
	   	image = '/assets/img/icon-location.png';

	for (var i = 0; i < maps.length; i++){
		var mapBlock = maps[i];
		coordLng = + mapBlock.getAttribute('data-longitude');
		coordLat = + mapBlock.getAttribute('data-lattitude');
		var myLatLng = {lat: coordLat, lng: coordLng};
		...
		$('.city .contacts-block').each(function(){
			lat = + $(this).attr('data-lattitude');
	...
	$('.js_city').on('change', function(){
		var attrAnch = $(this).find('.selected').data('type');
