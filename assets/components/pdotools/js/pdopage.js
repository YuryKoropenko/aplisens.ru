...
pdoPage.initialize = function (config) {
    if (pdoPage.keys[config['pageVarKey']] == undefined) {
        ...
        if(typeof mse2Config != "undefined" && mse2Config.page > 1){
            tpage = mse2Config.page;
        }
...
pdoPage.loadPage = function (href, config, mode) {
    ...
    if(config['mode'] == "filter"){
        ...
        var mse2Params = mSearch2.getFilters();
        mSearch2.Hash.set(mse2Params);
        mSearch2.load('','','append');
        return true;
    }
